...
func main() {
	...
	// Start the pricing API server on port 8082 for Funky integration
	pricingMux := http.NewServeMux()
	pricingMux.HandleFunc("/pricing/aws", awsPricingHandler(dbHandler))
	pricingMux.HandleFunc("/pricing/azure", azurePricingHandler(dbHandler))
	pricingMux.HandleFunc("/pricing/unified", unifiedPricingHandler(dbHandler))
	
	go func() {
		log.Printf("Starting pricing API server on http://localhost:8082/")
		log.Printf("Pricing endpoints available:")
		log.Printf("  - http://localhost:8082/pricing/aws")
		log.Printf("  - http://localhost:8082/pricing/azure")
		log.Printf("  - http://localhost:8082/pricing/unified")
		if err := http.ListenAndServe(":8082", pricingMux); err != nil {
			log.Fatalf("Failed to start pricing API server: %v", err)
		}
	}()

	log.Printf("Starting main server on http://localhost:%s/", port)
...
// Pricing API handlers for Funky integration
func awsPricingHandler(dbHandler *database.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		computePrices := getAWSComputePrices(dbHandler, region)
		for k, v := range computePrices {
			response.Compute[k] = v
		}

		// Get storage pricing
		storagePrices := getAWSStoragePrices(dbHandler, region)
...
func azurePricingHandler(dbHandler *database.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		region := r.URL.Query().Get("region")
		if region == "" {
			region = "eastus"
		...
		// Get compute pricing from raw data
		computePrices := getAzureComputePrices(dbHandler, region)
		for k, v := range computePrices {
			response.Compute[k] = v
		}

		// Get storage pricing
		storagePrices := getAzureStoragePrices(dbHandler, region)
		for k, v := range storagePrices {
			response.Storage[k] = v
		}
...
func unifiedPricingHandler(dbHandler *database.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response := map[string]interface{}{
			"aws": PricingResponse{
				Provider: "aws",
				Region:   awsRegion,
				Compute:  getAWSComputePrices(dbHandler, awsRegion),
				Storage:  getAWSStoragePrices(dbHandler, awsRegion),
				Transfer: map[string]float64{"in": 0.0, "out": 0.09},
			},
			"azure": PricingResponse{
				Provider: "azure",
				Region:   azureRegion,
				Compute:  getAzureComputePrices(dbHandler, azureRegion),
				Storage:  getAzureStoragePrices(dbHandler, azureRegion),
...
func getAWSComputePrices(dbHandler *database.DB, region string) map[string]float64 {
	...
	for _, instanceType := range instanceTypes {
		...
		var price float64
		err := dbHandler.GetConn().QueryRow(query, instanceType, location).Scan(&price)
		if err == nil && price > 0 {
...
func getAWSStoragePrices(dbHandler *database.DB, region string) map[string]float64 {
	prices := make(map[string]float64)
	...
	for key, storageClass := range storageClasses {
		...
		var price float64
		err := dbHandler.GetConn().QueryRow(query, "%"+storageClass+"%", location).Scan(&price)
		if err == nil && price > 0 {
			prices[key] = price
		}
	}
...
func getAzureComputePrices(dbHandler *database.DB, region string) map[string]float64 {
	prices := make(map[string]float64)
	...
	for _, vmSize := range vmSizes {
		...
		var price float64
		err := dbHandler.GetConn().QueryRow(query, region, vmSize).Scan(&price)
		if err == nil && price > 0 {
...
func getAzureStoragePrices(dbHandler *database.DB, region string) map[string]float64 {
	prices := make(map[string]float64)
	...
	for key, meterName := range storageQueries {
		...
		var price float64
		err := dbHandler.GetConn().QueryRow(query, region, "%"+meterName+"%").Scan(&price)
		if err == nil && price > 0 {
			prices[key] = price
		}
	}
